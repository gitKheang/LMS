...
export class MockApiService implements LibraryApi {
  ...
  async getCategories(): Promise<string[]> {
    ...
    const categories = Array.from(
      new Set(books.filter((b) => b.isActive).map((b) => b.category))
    );
  ...
  async updateBook(
    ...
    Object.assign(
      book,
      { ...rest, imageUrl: sanitizedImage || undefined },
      { updatedAt: new Date().toISOString() }
    );
  ...
  resetLibraryData() {
    ...
    bookCopies.splice(
      0,
      bookCopies.length,
      ...this.clone(this.initialBookCopies)
    );
  ...
  async getAdminLoans(): Promise<LoanWithRelations[]> {
    await this.simulateDelay();
    return this.clone(
      loans.map((loan) => this.enrichLoan(this.refreshLoanStatus(loan)))
    );
  }
  ...
  async requestPasswordReset(email: string): Promise<void> {
    ...
    if (user.role !== "USER") {
      throw new Error(
        "Only student accounts can request a reset through this form"
      );
    }
  ...
  async createLoan(payload: CreateLoanPayload): Promise<LoanWithRelations> {
    await this.simulateDelay();
    const availableCopy = bookCopies.find(
      (copy) => copy.bookId === payload.bookId && copy.status === "AVAILABLE"
    );
    if (!availableCopy) {
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const overdueLoans = loans.filter(
      (loan) => this.refreshLoanStatus(loan).status === "OVERDUE"
    );
    ...
    const recentLoans = loans
      ...
      .sort(
        (a, b) =>
          parseISO(b.borrowDate).getTime() - parseISO(a.borrowDate).getTime()
      )
      .slice(0, 5)
      .map((loan) => this.enrichLoan(loan));
  ...
  async getAvailableBooksForLoans(): Promise<BookWithAvailability[]> {
    ...
    return this.clone(
      books
        .filter(
          (book) => book.isActive && this.getAvailableCopies(book._id) > 0
        )
