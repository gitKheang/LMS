...
export class AdminBooksComponent implements OnInit {
  ...
  async loadBooks() {
    ...
    try {
      const books = await this.api.getAdminBooks(
  ...
  handleImageSelected(event: Event) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      this.toast.show({
        ...
        description: "Cover images must be smaller than 2MB.",
        variant: "destructive",
      });
      input.value = "";
      return;
  ...
  async submit() {
    ...
    try {
      if (this.editingBook()) {
        ...
        this.toast.show({
          title: "Book updated",
          description: `Successfully updated ${value.title}`,
        });
      } else {
        ...
        this.toast.show({
          title: "Book created",
          description: `Successfully created ${value.title}`,
        });
      }
      this.closeDialog();
